...
class PropertyController extends Controller
{
        ...
        if($step == 'basics'){
            ...
            if($_POST){
                $rules = array(
                    ...
                    'weekly_discount' => 'numeric|max:99|min:0',
                    'monthly_discount' => 'numeric|max:99|min:0'
                );
    ...
    public function single(Request $request){
        ...
        $data['property_photos']     = PropertyPhotos::where('property_id', $request->id)->get();
        ...
        $data['checkin'] = (isset($request->checkin) && $request->checkin != '')?date('m/d/Y', strtotime($request->checkin)):'';
        $data['checkout'] = (isset($request->checkout) && $request->checkout != '')?date('m/d/Y', strtotime($request->checkout)):'';
