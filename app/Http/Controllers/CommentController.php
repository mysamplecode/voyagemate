...
class CommentController extends Controller
{
  ...
  {
  	//$comment = Comment::where('photo_id', $request->photo_id)->get();
    ...
    if(\Auth::check()){
      ...
      <div class="share_D">
        <a href="https://www.facebook.com/sharer/sharer.php?u='.url('photo/single/'.$request->photo_id).'" target="_blank"><img src="'.url('public/front/img/icon/facebook-icon.png').'" alt="" style="margin-right:1%;"/></a>
        <a href="http://twitter.com/share?url='.url('photo/single/'.$request->photo_id).'&text='.$photo->title.'" target="_blank""><img src="'.url('public/front/img/icon/twitter-icon.png').'" alt="" style="margin-right:1%;"/></a>
        <a href="mailto:?Subject='.$photo->title.'&Body='.$photo->description.' '.url('photo/single/'.$request->photo_id).'"><img src="'.url('public/front/img/icon/email.png').'" alt=""/></a>
        </div>
      </div>
      ...
      $cont .= '<div class="user-comments">
          ...
          foreach ($photo->comments as $key => $value) {
              $delete_div = '';
              if(\Auth::check() && $value->user_id == \Auth::user()->id) $delete_div = '<div class="pull-right"><a href="javascript:void(0)" class="cmnt-del" data-rel="'.$value->id.'">'.trans('messages.utility.delete').'</a></div>';
          ...
          $cont .= '</div>';
          if($photo->comments->count() > 3) $cont .= '<div class="mtop20 pinkColor" style="text-align:center;"><a id="load-more-comment" href="javascript:void(0)">'.trans('messages.utility.load_more').'</a></div>';
      if(\Auth::check()){
  ...
  public function save_comments(Request $request){
    ...
    $photo = Photo::where('id', $request->photo_id)->first();
    //$notification = '<p><a href="'.url('profile/'.\Auth::user()->id).'">'.ucfirst(\Auth::user()->name).'</a> commented on a <a href="'.url('photo/single/'.$request->photo_id).'">photo</a> of yours</p>';
