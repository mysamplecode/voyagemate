...
class SearchController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        //$geocode      = @file_get_contents($map_where);
        ...
        $data['default_min_price'] = $this->helper->convert_currency('USD', '', 5);
    ...
    function search_result(Request $request)
    {
        ...
        {
            if($property_type != '') $property_type = explode(',', $property_type);
            else $property_type = [];
        }

        if(!is_array($space_type))
        {
            if($space_type != '') $space_type = explode(',', $space_type);
            else $space_type = [];
        }

        if(!is_array($amenities))
        {
            if($amenities != '') $amenities = explode(',', $amenities);
            else $amenities = [];
        }
        ...
        if($bedrooms) $properties_where['properties.bedrooms']  = $bedrooms;

        if($bathrooms) $properties_where['properties.bathrooms'] = $bathrooms;
            
        if($beds) $properties_where['properties.beds']      = $beds;
        ...
        //echo json_encode($properties_whereIn);exit;
        ...
        $properties = $properties->paginate(18)->toJson();
        echo $properties;
    }
