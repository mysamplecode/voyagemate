...
<?php $__env->startPush('scripts'); ?>
  <script type="text/javascript">
    ...
    function inbox_data_load(page = 1, type='all'){
        ...
        success: function (result) {
          ...
          for (var key in messages) {
            if (messages.hasOwnProperty(key)) {
              ...
              if(messages[key].host_user ==1 && messages[key].bookings.status == 'Pending') view_url = "<?php echo e(url('booking')); ?>/"+ messages[key].booking_id;
              else view_url = "#";
          ...
          $('#inbox-page-info').html(result.from+' – '+result.to+' of '+result.total+' Messages');
    ...
    function inbox_message_type_load(type='all'){
      var filter_options = '';
      ...
      $.ajax({
        ...
        dataType: 'json',
        success: function (result) {
          filter_options = '<option value="all" selected="selected" >'+"All messages"+' ('+result.all_message_count+')</option>'
          
          +'<option value="unread">'+"Unread"+' ('+result.unread_count+')</option>'
          +'<option value="accepted_requests">'+"Accepted Requests"+' ('+result.accepted_count+')</option>'
          +'<option value="pending_requests">'+"Pending Requests"+' ('+result.pending_count+')</option>'
          +'<option value="cancelled_requests">'+"Cancelled Requests"+' ('+result.cancelled_count+')</option>'
          +'<option value="declined_requests">'+"Declined Requests"+' ('+result.declined_count+')</option>'
          +'<option value="expired_requests">'+"Expired Requests"+' ('+result.expired_count+')</option>'
