            ...
            <form action="{{ url('admin/bookings/detail/'.$result->id) }}" method="post" class='form-horizontal'>
              <div class="box-body">
                ...
                <div class="form-group">
                  ...
                  <div class="col-sm-6 col-sm-offset-1">
                    {{ $result->currency->symbol }}{{ $result->base_price  }}
                   </div>
                </div>
                <div class="form-group">
                  <label class="col-sm-3 control-label">
                  ...
                  <div class="col-sm-6 col-sm-offset-1">
                    {{ $result->currency->symbol }}{{ $result->cleaning_charge }}
                   </div>
                </div>
                <div class="form-group">
                  <label class="col-sm-3 control-label">
                  ...
                  <div class="col-sm-6 col-sm-offset-1">
                    {{ $result->currency->symbol }}{{ $result->guest_charge }}
                   </div>
                </div>
                <div class="form-group">
                  <label class="col-sm-3 control-label">
                  ...
                  <div class="col-sm-6 col-sm-offset-1">
                    {{ $result->currency->symbol }}{{ $result->security_money }}
                   </div>
                </div>
                <div class="form-group">
                  <label class="col-sm-3 control-label">
                  ...
                  <div class="col-sm-6 col-sm-offset-1">
                    {{ $result->currency->symbol }}{{ $result->service_charge }}
                   </div>
                </div>
                <div class="form-group">
                  <label class="col-sm-3 control-label">
                ...
                <div class="form-group">
                  ...
                  <div class="col-sm-6 col-sm-offset-1">
                    {{ $result->currency->symbol }}{{ $result->host_penalty_amount }}
                   </div>
                </div>
                @endif
                @if($penalty_amount != 0)
              ...
              @if((($result->status == 'Accepted' &&  $result->checkin_cross == 0 ) || $result->status == 'Cancelled') && $result->check_host_payout != 'yes' && $result->admin_host_payment != 0)
                @if($result->host_account)
                  <form action="{{ url('admin/bookings/pay') }}" method="post">
                    ...
                    <div class="text-center"> 
                      <button type="submit" class="btn btn-primary" onclick="return confirm('Are you sure you want to mark it as paid?')">Paid to Host({{ $result->currency->symbol }}{{ $result->admin_host_payment }})</button>
                    </div>
                  </form>
                @else
              ...
              @if(($result->status == 'Declined' || $result->status == 'Cancelled' || $result->status == 'Expired') && $result->check_guest_payout != 'yes' && $result->admin_guest_payment != 0)
                @if($result->guest_account)
                  <br>
                  <form action="{{ url('admin/bookings/pay') }}" method="post">
                    <input type="hidden" name="booking_id" value="{{ $result->id }}">
                    ...
                    <div class="text-center">
                      <button type="submit" class="btn btn-primary" onclick="return confirm('Are you sure you want to mark it as paid?')">Paid to Guest({{ $result->currency->symbol }}{{ $result->admin_guest_payment }})</button>
                    </div>
                  </form>
                @else
