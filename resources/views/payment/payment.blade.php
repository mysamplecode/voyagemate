  ...
  <div class="col-md-4 col-sm-4 col-xs-12 mb10">
    ...
    <div class="panel panel-default">
      <div class="panel-body">
        ...
        <hr/>
        <div class="exfont">
          <div class="side_tt">{{ $result->property_price->currency->symbol }}{{ $price_list->property_price }} x {{ $nights }} {{trans('messages.payment.nights')}}</div>
          <div class="side_tt text-right">{{ $result->property_price->currency->symbol }}{{ $price_list->total_night_price }}</div>
        </div>
        @if($price_list->service_fee)
        <div class="exfont">
          <div class="side_tt">{{trans('messages.payment.service_fee')}}</div>
          <div class="side_tt text-right">{{ $result->property_price->currency->symbol }}{{ $price_list->service_fee }}</div>
        </div>
        @endif
        @if($price_list->additional_guest)
        <div class="exfont">
          <div class="side_tt">{{trans('messages.payment.additional_guest_fee')}}</div>
          <div class="side_tt text-right">{{ $result->property_price->currency->symbol }}{{ $price_list->additional_guest }}</div>
        </div>
        @endif
        @if($price_list->security_fee)
        <div class="exfont">
          <div class="side_tt">{{trans('messages.payment.security_deposit')}}</div>
          <div class="side_tt text-right">{{ $result->property_price->currency->symbol }}{{ $price_list->security_fee }}</div>
        </div>
        @endif
        @if($price_list->cleaning_fee)
        <div class="exfont">
          <div class="side_tt">{{trans('messages.payment.cleaning_fee')}}</div>
          <div class="side_tt text-right">{{ $result->property_price->currency->symbol }}{{ $price_list->cleaning_fee }}</div>
        </div>
        @endif
        <div class="clearfix"></div>
        <hr/>
        <div class="ex-pop mb20">
          ...
          <div class="side_tt text-right"><strong>{{ $result->property_price->currency->symbol }} {{ $price_list->total }}</strong></div>
          <div class="clearfix"></div>
        </div>
        <p class="exfont">
          <small>
            ...
            <strong><span id="payment-total-charge">&euro;{{ $price_eur }}</span></strong>.
            {{trans('messages.payment.exchange_rate_booking')}} &euro; 1 to {{ $result->property_price->currency->org_symbol }}{{ $price_rate }} {{ $result->property_price->currency_code }} (your host's listing currency).
          </small>
        </p>
      </div>
    </div>
